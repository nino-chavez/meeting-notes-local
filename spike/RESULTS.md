...
[`aec-bound-results-redacted.json`](./aec-bound-results-redacted.json) holds all
five experiments below with per-window scores and SHA-256 digests of every
recording and segment list, so a figure here can be checked rather than taken.
Transcript text was removed from the tracked snapshot; it is unnecessary for
recomputing these figures and cannot belong in a public repository.
...
python3 spike/aec_bound.py --label acceptance \
  ...
  --out ~/enroll-calibrated/aec-bound-results.json
```
