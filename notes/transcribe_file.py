...
def inside_repo(path: Path, repo: Path | None = None) -> bool:
    """Whether an output path lands inside this repository."""
...
def main() -> int:
    ...
    if inside_repo(args.out):
        p.error(
            f"--out {args.out} is inside the repository. A transcript is the "
            "same secret as its audio. Write it beside the recording instead."
        )
