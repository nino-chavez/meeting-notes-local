...
<p><strong>DRAFT. Not human-approved.</strong> Review all {len(reference["events"])} events and all {len(reference["sections"])} transcript sections. Use the section note to record any decision, action, proposal or unresolved question the draft missed. Exported decisions still require command-line validation and a separate operator lock.</p>
<p>Minimum acceptable-anchor hitting set: {reference["gate"]["minimum_keep_required"]}. Keep limit: {KEEP_LIMIT}. Headroom: {reference["gate"]["headroom_before_lock"]}.</p>
...
<section aria-label="Events">{''.join(event_html)}</section><section aria-label="Transcript sections">{''.join(sections)}</section>
