...
| C. Trust foundation | In progress. Active meetings are excluded from retention under a serialized storage scan, a process-lifetime writer lock closes the second-process storage path, and an independently audited coordinator now durably restores one withheld turn through request, re-inspected result, meeting-pointer update, and exact terminal commit. Fresh recovery covers every durable phase; a simultaneous storage mutation is quarantined without worker work or pointer change. The coordinator remains unregistered. Profile controls and immediate/staged deletion actions remain. | Add the profile and deletion controls, then exercise a real withheld-turn restoration before admitting or registering the command. | Retention-policy wording and far-end-notice choices; real withheld-turn restore. | Cumulative 1–2 weeks |
| D. Evidence-linked automatic notes | In progress. An audited, unadvertised worker seam can publish only an exact passing `note/2` JSON/Markdown pair from an injected generator, repair identical crash orphans, and refuse rejected or misbound candidates. No generator is admitted and no meeting pointer is updated. | Add the fixed generator/model identity, coordinator commit, locator landing, and transcript-only rejected fallback; deterministic checks precede private semantic review. | Semantic support and usefulness adjudication. | Additional 2–3 weeks |
