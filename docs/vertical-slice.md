...
| 4 | Evidence-linked notes | **Intervention eight: a conditional gate — but only with an unadvertised control fixture in the same change** | Intervention seven ran 2026-08-07 (`mlx-note-matrix/3`, 13 fixtures, `notes/mlx_note_matrix_receipt_arms_recorded.json`). The prediction held: given "If we decided to ship Tuesday, we would need Dana", the model **accepted** it, exactly as the deterministic arm does. **Both arms fail the same way — neither can see a counterfactual** — and the citation gate passed, because the model cited the real sentence. Correct citation, wrong inference; no locator gate could catch it. In product terms: that meeting would produce a note recording a decision to ship Tuesday. **The pass count is 11 of 13 but the graded count is unchanged at 10 of 12** — the increase is the ungraded fixture, which cannot fail, and a test holds the number of ungraded fixtures at one. No pre-existing fixture's request or response digest moved, checkable for the first time. `admits` stays false. |
