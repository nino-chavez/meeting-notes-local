...
| 4 | Evidence-linked notes | **Polarity gate first, then** the identifier truncation — the order is the point | **Reordered 2026-08-07.** Reading `read_semantic_support.py`'s sheet for the first time found that fixture `negation-proposal` produces the claim "merge the red branch" from the evidence "I propose that we do **not** merge the red branch" — the opposite of what it cites. No registered gate catches it; all three matrix failures carry `citation-locator` and nothing else. **So the identifier fix must not ship alone:** landing it would remove the only thing refusing that claim, taking the matrix from 9 of 12 to 12 of 12 while the product got worse. Intervention four is preregistered in `notes/MLX_NOTE_ADMISSION.md` as the polarity gate, with `negation-decision` as the control that must keep passing. Neither needs a human; only admission does. |
