...
renders the survey-led sales/discovery note reader with a derived summary, typed output
sections, stated/inferred labels, an explicit action owner, and exact-word evidence.
It also connects manual Start, per-attempt consent, healthy/degraded ambient capture,
Stop, local transcription, and complete/partial meeting joins without turning status
into a destination. The next trust slice keeps Settings outside primary navigation,
requires both permission fixtures and an explicit no-default retention choice, blocks
Start when readiness is incomplete, and resets the owner voice profile without
deleting meetings. Guided enrolment now continues through two separated operator
sittings, permitted comparison speech, an unselected measured policy, build/private
persistence, completion, and discard. Correction/recovery now continues through a
restored transcript view, visibly stale note history, a request-only interruption,
fresh document reload, and safe regeneration retry. The receipt contains only
synthetic fixture IDs and version numbers; no meeting text is stored. These synthetic
slices are ready for review but have not been judged useful or validated against the
installed runtime.
...
| C. Trust foundation | In progress, with the fixed-slot lifecycle join parked as an audited implementation candidate rather than installed Preview work. The independent audit found two release blockers: initial slot publication can mutate a pathname before descriptor identity is proved, and receipt validation accepts impossible phase histories. The candidate also overstates volume readiness and must not be promoted. The walking skeleton now places conjunctive readiness, retention choice, reset, and guided multi-sitting enrolment through private persistence/discard in the accepted shell. Capture admission and the frozen alpha command boundary are unchanged. | Review the enrolment policy and lifecycle interaction, then resume descriptor-before-mutation publication, legal transition validation, strict encoder validation, and installed trust joins. | Retention-policy wording and far-end-notice choices; real profile policy, deletion, reset and withheld-turn decisions. | Cumulative 1–2 weeks after the design gate |
...
| E. Product surfaces and retrieval | The composition gate passed on 2026-08-02 at exact prototype commit `689fcda396c073d21d69d8feb3cfb6031bec0596`: Find opens by default and Find, Meetings, and Promises remain persistent navigation. Runnable discovery-note and ambient-capture slices now join Settings readiness, retention, reset, complete synthetic enrollment/discard, and correction through a request-only interruption, fresh reload, and regeneration retry. Mechanical browser evidence covers the full recovery path at 960×900 with no outer overflow and no console errors. No prototype writes product records or reads private meetings. | Review the content/capture/trust/recovery journey, then move accepted slices into thin installed Preview increments. The next independent research item is the non-blocking MLX word-timestamp and fidelity sidecar benchmark. | Automatic-note semantic support/usefulness, consent sufficiency, profile policy, hardware capture, native recovery, and installed behavior remain human/runtime gates; navigation composition is accepted. | Interaction review now; implementation remains within the cumulative 2–3 week surface range |
