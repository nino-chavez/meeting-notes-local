...
independent authority. The crate-private lifecycle transition and its recovery matrix
are now implemented and tested; Preview exposure remains blocked on the strict-loader
bridge and the guided evidence-capture surface. Its `writing` and `ready` payloads have
...
until the canonical Python loader has admitted the exact staged candidate digest; a
caller assertion is not profile authority.
