...
| C. Trust foundation | In progress. The independently audited restoration coordinator covers every durable phase and refuses simultaneous storage mutation. The development-only profile bridge validates quarantined candidates without following links, trusting unsafe ownership or modes, overwriting installed bytes, or crashing on malformed data. The Preview meeting detail now shows validated retention state, scheduled/manual policy, exact retained two-leg byte count, and the consequences of released audio without exposing paths or adding mutation authority. The audited immediate `audio-deletion/1` facade remains private and unregistered; adding its irreversible Preview command is a separate explicit-approval gate. The frozen alpha command boundary is unchanged. The durable profile-reset contract is frozen; reset and guided enrolment remain. | After explicit capability approval, add the two-step Preview audio-release action over the existing facade. Then implement reset/enrolment and exercise real profile, withheld-turn, and reviewed deletion paths before beta admission. | Retention-policy wording, irreversible deletion capability, and far-end-notice choices; real profile and withheld-turn decisions. | Cumulative 1–2 weeks |
| D. Evidence-linked automatic notes | In progress. The private coordinator and inspect-only transport remain closed and recovery-tested. A synthetic-only MLX note-admission harness resolves exact canonical locators outside the model and fails closed to `transcript-only`. SmolLM2 1.7B and Qwen2.5 1.5B remain unadmitted, but independent audit found their model-level rejection evidence inconclusive: the prompt advertised a different JSON root than the strict parser required, the retained failure category could not prove non-JSON, and Qwen's reported warm latency timed a whole suite plus repeated model hashing rather than warmed inference. Citation, locator, semantic, and human gates were never reached. No generator or model is wired into Preview. | Align the strict prompt/parser contract and identity/timing receipts, then rerun only one supported and one empty pinned-Qwen fixture before any new candidate search. Admit a create bridge only after typed-output, exact-locator, latency, memory, repeatability, and human semantic/usefulness gates pass. | Semantic support and usefulness adjudication. | Additional 2–3 weeks after a registered experiment passes |
| E. Product surfaces and retrieval | In progress. The separate Preview bundle now registers the bounded reader commands and renders the real Library, exact search, meeting detail, typed `note/2` claims when present, canonical evidence landing, and transcript-only fallback in the original light/editorial design system. The frozen alpha command boundary is unchanged. The default 960×900 window shows the current Library without scrolling. Searches and returns rebuild one current handle generation; metadata-only meetings cannot receive transcript actions; exact Unicode scalar spans survive search reopening; and transcript opens recheck the validated digest, path, and parsed bytes while holding storage coordination. Correction/regeneration commands remain unregistered. | Continue note-reader and trust-action slices, beginning with visible retention/disk state and the already-audited immediate audio-release path. Pass the synthetic corpus and cold interaction review before beta admission. | Cold operator review of the working surfaces. | Additional 2–3 weeks, partly parallel with D |
