...
| D. Evidence-linked automatic notes | In progress. The private coordinator and inspect-only transport remain closed and recovery-tested. A synthetic-only MLX note-admission harness now resolves exact canonical locators outside the model and fails closed to `transcript-only`. Its first pinned 1.7B candidate measurement was rejected: three runs produced the same malformed non-JSON response, so claim, locator, and semantic gates were never reached. No generator or model is admitted or wired into Preview. | Select and preregister the next locally licensed candidate without changing the shipped transcription runtime. Admit a create bridge only after typed-output, exact-locator, latency, memory, repeatability, and human semantic/usefulness gates pass. | Semantic support and usefulness adjudication. | Additional 2–3 weeks after a registered experiment passes |
