...
| E. Product surfaces and retrieval | In progress. The original light/editorial Preview now has a delivered Library → meeting detail → canonical transcript reader loop at `4598389`, with exact search, digest/path/parsed-byte rechecks, and a 960×900 visual acceptance size. Typed `note/2` claims and canonical claim evidence remain synthetic/contract evidence only; the projector and model path are not admitted to real Preview. The voice surface exposes content-free status, preserve-first legacy migration, separately confirmed lifecycle-backed reset, and the deterministic guided-enrolment shortfall; profile activation and the enrolment command remain unavailable, and the surface says which requirements it cannot decide. Correction/regeneration commands remain unregistered. | Continue continual Preview delivery with the dedicated sitting recorder that feeds the guided-enrolment evaluator, then withheld-turn restoration and stale-note regeneration. Pass synthetic state tests and cold interaction review before beta admission. | Cold operator review of the working surfaces. | Additional 2–3 weeks, partly parallel with D |
