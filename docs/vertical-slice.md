...
| 3 | Audio has a stated lifetime | Whole-meeting deletion — **core landed 2026-08-07**, shell command is the next slice | The core authority, the `meeting-deletion/1` receipt, its three-state machine and startup reconciliation exist in `crates/session-core/src/meeting_deletion.rs` with 11 tests. Still buildable without a human: the shell command, which needs an authorization path beside the existing audio one in `library_reader`, a capability, the contract pins, and the §G two-step confirmation. **Exercising it remains gated:** "exercise real destructive actions only as operator actions before beta admission", and wave C's gate names real deletion decisions — so no real `meeting-deletion/1` receipt exists and the synthetic tests do not advance that gate. The policy wording is a separate item below; carrying the two as one row is what made a buildable thing look wholly gated. |
| 4 | Evidence-linked notes | The fourth registered intervention against the identifier truncation | **Added 2026-08-07.** Three interventions ran on 2026-08-06 and the mechanical picture narrowed to one failure on three fixtures: `sf-` plus 64 hex, cut at 67 characters. The margin receipt shows one decision losing three times at the same step with the same runner-up token `-t` (−1.11, −0.52, −0.31), so this is a single characterized mechanism, not a search. Preregistering and running the next intervention needs no human input — only the eventual admission does. |
