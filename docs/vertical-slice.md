...
| E. Product surfaces and retrieval | In progress. The separate Preview bundle now registers the bounded reader commands and renders the real Library, exact search, meeting detail, typed `note/2` claims when present, canonical evidence landing, and transcript-only fallback in the original light/editorial design system. The frozen alpha command boundary is unchanged. The default 960×900 window shows the current Library without scrolling. Independent audit found and the next Preview closed two interaction defects: every search/return now rebuilds one current handle generation, and metadata-only meetings cannot receive transcript handles or actions. Correction/regeneration commands remain unregistered. | Preserve the exact search span through transcript landing, bind transcript opens to the validated digest before correction can mutate it, then continue note-reader and trust-action slices. Pass the synthetic corpus and cold interaction review before beta admission. | Cold operator review of the working surfaces. | Additional 2–3 weeks, partly parallel with D |
