...
| 4 | Evidence-linked notes | **Record `request_sha256` per fixture in the matrix receipt**, then investigate the identifier truncation still failing two fixtures | **The matrix is re-runnable again as of 2026-08-07** and was run on a fresh environment the same day (`notes/mlx_note_matrix_receipt_wheel_pin.json`): `every_fixture_ran: true`, ten of twelve fixtures passing every registered gate against nine on 2026-08-06. `RECORD` is out of `MLX_RUNTIME`; the fresh environment matched `METADATA` 3 of 3 and would have been refused 3 of 3 by the old pin. **Intervention four's prediction was falsified in an informative direction:** `negation-proposal` passes rather than carrying `claim-polarity`, because the rule is advertised in `response_contract` — a request key — so the model read it and stopped producing the inverted claim. That also makes intervention four's "this changes no request" wrong, corrected in place. **The next build follows from the second defect this exposed:** two consecutive preregistrations made claims about request digests that the matrix receipt does not record, so neither could be checked from a receipt. `admits` stays false and no generator is wired into Preview. |
