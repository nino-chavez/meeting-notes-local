...
| 4 | Evidence-linked notes | **Blocked 2026-08-07 — the registered runtime cannot be rebuilt.** Not an operator decision; a defect in the pin | The polarity gate landed and is unit-tested. Verifying its effect needs a matrix run, and the matrix refuses: `MLX_RUNTIME` pins each package's installer-written `RECORD` digest, which varies by installer and by byte-compilation. Three environments on one machine, all with matching Python and package versions, reproduced `METADATA` 9 of 9 and `RECORD` **1 of 9**. Every future intervention on this path is blocked until the pin moves off `RECORD` and onto the wheel's own digest — recorded in `notes/MLX_NOTE_ADMISSION.md`, and a change to the registered runtime contract, so it needs preregistering. Fixing the pin is buildable now and needs no human. |
