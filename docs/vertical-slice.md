...
| 4 | Evidence-linked notes | **Build a fresh environment and run the 12-fixture matrix.** The pin was fixed 2026-08-07; nothing else blocks the run | `RECORD` is out of `MLX_RUNTIME` — it is installer-written and varied 8 of 9 times across three environments, while wheel-shipped `METADATA` matched 9 of 9. Three tests pin the new shape, and one pins that no request digest depends on the runtime identity, so the committed 2026-08-06 receipts stay comparable. Intervention five is preregistered in `notes/MLX_NOTE_ADMISSION.md` with the falsifier: if any request digest moves, the change is withdrawn. **The rebuild itself is unrun** — `mlx_lm` is not in this repo's `.venv`, and installing it here would alter the environment the committed receipts came from, so the run needs its own environment. Until it happens, nobody may call the matrix re-runnable or quote a count from it, including the polarity gate's predicted effect. |
