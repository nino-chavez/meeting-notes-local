...
| C. Trust foundation | In progress. Active meetings are excluded from retention under a serialized storage scan, a process-lifetime writer lock closes the second-process storage path, and audited primitives now store private immutable operation receipts and create/resolve one-turn transcript views. Results remain explicitly untrusted until artifacts are re-inspected. The storage-backed coordinator, meeting-pointer mutation/recovery, profile controls, and deletion actions remain. | Join the coordinator through artifact reinspection, meeting mutation, interruption, and fresh-process tests, then audit again before admission. | Retention-policy wording and far-end-notice choices; real withheld-turn restore. | Cumulative 1–2 weeks |
