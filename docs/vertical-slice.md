...
| 4 | Evidence-linked notes | **Add a second, distinct abstention fixture** — one that offers candidates and should still abstain | The matrix is re-runnable and the receipt now records `request_sha256` per fixture (`mlx-note-matrix/3`, `notes/mlx_note_matrix_receipt_request_digest.json`), which closes the gap where two preregistrations made request-digest claims no artifact could settle. Ten of twelve fixtures pass every registered gate. **Its first run found that the matrix has twelve fixtures and eleven distinct requests:** `abstain-chitchat` and `abstain-plain` produce a byte-identical request, because both yield zero candidates and only candidates reach the model. The empty-abstention path is pinned once, not twice. Two fixtures still refuse on the identifier truncation, which is the other open thread. `admits` stays false and no generator is wired into Preview. |
