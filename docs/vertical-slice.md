...
| 4 | Evidence-linked notes | **Intervention seven: let the two arms disagree without it counting as a fixture failure.** Preregistered, decided, not yet run | The matrix is re-runnable, ten of twelve fixtures pass, and the receipt records `request_sha256` per fixture. **The previous entry here was wrong and is withdrawn:** a second abstention fixture is not buildable. Only candidates reach the model, so *every* zero-candidate transcript builds one byte-identical request — the abstention path has a coverage ceiling of exactly one request, measured on four transcripts. And a candidate-bearing abstention fixture fails `control_expected` before the model is consulted, because the deterministic arm abstains only when there are no candidates. That exposed the finding worth keeping: the control accepts "If we decided to ship Tuesday, we would need Dana" as a decision — the same word-presence defect the polarity gate exists for, on the arm nothing gates. Intervention seven makes an arms-disagree outcome recordable rather than an automatic failure; it moves three registered surfaces and needs its own matrix run, so it is deliberately unbundled from the finding. `admits` stays false. |
