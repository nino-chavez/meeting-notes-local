...
one-day retention deadline is recorded locally; automatic deletion is closed.
Clean transfer remains open. Until it passes, the alpha is not cleared for
distribution. Automatic-note admission remains a separate, unmet
beta gate.
...
on download and again in `verify`, and installs `tokenizers==0.22.2` from its own
hash-pinned lock. The manifest lists all four, `corpus.embed` moved into
`ALPHA_OPERATIONS`, and `build_runtime.sh verify` now names
`LMN_EMBEDDING_MODEL_DIR` so the model-backed tests **run rather than skip** — a
suite that skipped the only tests touching the model would report the same green
as one that ran them.

**`corpus.embed` landed 2026-08-08** (US-13.10). It takes `{text_sha256, text}`
per window and returns `{text_sha256: base64 little-endian float32}` — the byte
layout the store already holds, so what gets written is what the model produced.
It reads no file and writes none. Windowing stays in `corpus_window.rs`, pinned to
the measured boundaries; a second cut in Python would be a third implementation of
it.
...
the script says so, and packaging it admits nothing. The embedding model is not in
that position: it is chosen and measured, and what is unjudged is whether 7 of 10
is useful, which no build mode can settle. A second optional component would have
taken three modes to four, and then to eight.

**What packaging does not settle, and is worth not mistaking.** The model being in
the bundle is not the model being any good. The number still standing is 7 of 10,
and 3 of 5 on the questions exact search cannot answer.
...
| ~~Which repair to preregister~~ | 4 | **Closed 2026-08-07, and it was never really open.** Both candidates ran on 2026-08-06, plus a third. Deferring this on 2026-08-06 cost nothing, because the work continued without the decision — which is itself the evidence that it was not an operator decision. |
...
**Feature 4 is the load-bearing item**, and the 2026-08-07 correction changed what
that implies. Features 5, 6, and 7 all converge on the admitted note generator, and
*admission* is a human call — but the mechanical work leading to it is not, and it
kept moving through three interventions on 2026-08-06 while this file recorded it as
waiting on a decision. Autonomous time goes to 4, 10, and 3. What actually needs the
operator is the semantic and usefulness adjudication at the end, and the two scope
decisions above — not the next experiment.

**The lesson this section keeps re-learning:** an item recorded as blocked-on-a-human
is expensive when it is wrong, because nobody works on it. Twice in two days a row
here named a decision that either was not the operator's (the repair choice) or was
separable from a build that could proceed (feature 3's wording). Before writing a row
into the operator table, name the specific thing only they can say.
...
|---|---|---|---|---|
| A. Alpha release closure | The unchanged signed alpha is byte-bound to the original capture attempt. Its one-day deadline, `2026-08-02T10:20:01-0500`, passed; a launch after that time completed automatic retention and an `audio-deletion/1` `removed` receipt (SHA-256 `59a500cb4f6c5e05e22425ba2f90c38629d300249b41628f5efb613bb029f4d5`) was observed at approximately `2026-08-02T20:24:47-0500`. Both bound microphone and system WAVs are absent; the exact retained transcript artifact remains present, JSON-readable, and digest-matched, without its text being inspected or emitted. This closes automatic deletion only. PR #2 and the release stay draft. **Corrected 2026-08-06: "clean Mac or account transfer remains open" was misleading and is withdrawn.** Transfer has happened repeatedly — the cohort DMG is distributed through R2, a cohort operator's report is what produced 0.3.1's one change, and Gatekeeper, notarization and stapling pass on every recorded build. What has never been written is the **closure receipt**: the three-block record under "Content-free closure receipt" in `docs/distribution-runbook.md` (`automatic_deletion`, `consented_hardware_run`, `clean_transfer`), bound to one unchanged build. A repository search on 2026-08-06 found no filled instance of it — the only receipts on disk are the MLX note probes. So the activity is done several times over and the evidence chain is empty, and because 0.2.2's evidence explicitly does not transfer to 0.3.0, each version restarts it. | Write the closure receipt against a build someone already has, rather than arranging another install. Then reconcile the draft PR and release record without inferring a release verdict. | Not another transfer. An operator recording the three-block receipt for one unchanged build — the interactive run is what is open, on 0.2.2, 0.3.0, 0.3.1 and 0.4.0 alike. | Under a day, once someone writes down a run they are already doing |
| B. Shared-contract freeze | Complete after two evidence-driven narrow corrections. Implementation proved that supported macOS exposes no descriptor-based executable launch, so the original audited wording required an impossible primitive. The corrected contract keeps the signed interpreter, standard library, and dynamic libraries inside the release trust boundary and descriptor-retains every manifest-listed bridge-controlled resource. Installed Preview storage also proved that macOS attaches `com.apple.provenance` to its private root, profile directory, and writer lock; the safe-tree predicate now permits only that non-authoritative platform marker and still refuses every other extended attribute or resource fork. All other correction/regeneration, worker, recovery, artifact, receipt, and fixture contracts remain frozen. **Third narrow correction, 2026-08-06: `app-runtime/1` gains a `permission_probe` resource.** First run may not claim a permission state it cannot measure (feature 10), and nothing could measure one — the microphone check lived inside the capture start path and the system-audio check is the status of the tap-create call, both reachable only by recording. `capture/permission-probe` measures them. Because the app *executes* it, and every other child this app executes is digest-verified from this manifest before it is spawned, leaving it out would have introduced the first unverified executed child. Registered rather than done quietly because the required set is checked exactly on both sides — `worker/main.py` compares `set(document) != required`, and Rust's `RuntimeManifest` is `deny_unknown_fields` — so writer and both readers move in one commit or the manifest fails closed. That fail-closed behaviour is the property being preserved, not a cost. The schema name is unchanged: the resource set widened, no existing entry's meaning moved, and no consumer reads a field differently than before. | Real projector transport must implement the corrected descriptor handoff, cancellation, and parent-death contract before admission. | None. | Complete 2026-08-01; storage-marker correction 2026-08-03 |
| C. Trust foundation | In progress. The independently audited restoration coordinator covers every durable phase and refuses simultaneous storage mutation. The development-only profile bridge validates quarantined candidates without following links, trusting unsafe ownership or modes, overwriting installed bytes, or crashing on malformed data. The macOS storage layer now has an independently audited, descriptor-bound profile primitive: full-chain no-follow directory opens, one-component `openat` files, closed owner/mode/link/flag/ACL/xattr/size checks, fixed zero-slot creation, identity observation, namespace-replacement refusal, and a retained-descriptor atomic file swap. The core profile lifecycle builds and reopens the fixed sequence-zero baseline: it binds `$APP_DATA` through every slot, publishes only the retained initializing directory, requires canonical envelope and payload digests, distinguishes a torn peer from coherent later authority, detects same-length in-place mutation, and refuses missing published state without repair. Existing live material without lifecycle authority remains untouched until the operator explicitly chooses preserve-first migration. That action binds the exact safe bytes into the baseline without activation or deletion; an unreceipted crash returns to review, while a receipt-bound crash resumes safely. Reset now uses the same alternating journal and writer/sequence authority: it writes `deleting` before swapping the live and fixed zero slots, writes `staged` before truncating only the retained original profile descriptor, and writes `removed` once. Fresh startup recovers the documented pre-swap, post-swap, staged, and post-truncate rows; an impossible staged-before-swap row quarantines without mutation. The same crate-private journal now implements enrolment `writing`, `ready`, and `active`: startup truncates only the exact uncommitted staged inode, resumes either ready-side swap layout, and never promotes bytes without a ready digest. Reset can follow an active enrolled profile while carrying forward exact reset history. The strict-loader coordinator now binds the canonical worker verdict to the descriptor-reopened candidate digest, publishes only matching bytes, and keeps digest-bound candidate cleanup separately retryable after `active`. Preview exposes reset only after a separate visible confirmation and names that meetings remain. The raw lifecycle mutators remain crate-private; their process capability borrows the writer lock, holds the global storage sequence through completion, accepts no caller storage root, and refuses before mutation while any meeting is active. The deterministic half of guided enrolment now exists as `session-core::enrollment_guidance`: a content-free evaluator that names every unmet enrolment requirement in the term the capture gate enforces, with its constants re-derived from `spike/speaker_gate.py` and pinned by test rather than copied from the screen inventory's prose. Preview renders it, and the cached voice status now carries profile presence and profile activation separately, so an active enrolled profile can no longer inherit the preserved-legacy copy that promises Preview did not activate what it found. No dedicated sitting is recorded yet, so the evidence set is empty and the surface truthfully reports `blocked` with the first enforced step. Enrolment remains unexposed as a command, the Preview capability still grants no enrolment or profile-activation permission, and the frozen internal-alpha worker operation set is unchanged. Preview invokes lifecycle authority once during startup and caches only a content-free result for Settings: baseline ready with profile presence, migration review required, needs attention, or unavailable. Opening Settings performs no lifecycle mutation or profile read. Migration and profile-attention states do not change startup, retention, capture admission, or current one-operator recording availability. Losing the bound app-data root or writer-lock authority remains a global startup failure rather than being mislabeled as profile damage. After explicit operator approval, the Preview also exposes a two-step, one-meeting audio-deletion action over the audited staged `audio-deletion/1` facade. A dedicated single-use handle, fresh retention recheck, process writer lock, and core meeting lease keep that action narrower than generic meeting or transcript authority. Synthetic tests prove profile reset leaves meeting storage untouched and released audio leaves transcript and note artifacts intact; no real profile-reset receipt is claimed. The frozen alpha command boundary is unchanged. Guided multi-session enrolment, withheld-turn restoration, policy change, and whole-meeting deletion remain. | The crash-safe sitting evidence store now exists (`session-core::sitting_evidence`): durable write-once evidence rows, a digest-bound three-state cleanup receipt in the `audio-deletion/1` shape, rehearsal labels for anything deleted before derivation, startup reconciliation, and a Preview snapshot that reads it under the writer lock and refuses to evaluate recorded evidence without the manifest's verified encoder digest. The recorder surface renders that store read-only: per-sitting lifecycle states in content-free copy plus the honest recording boundary derived from the build's own placeholder-encoder signal; enrolment mutation stays ungranted. The worker-side derivation operation now exists on the boundary lane only (`sitting.derive`: whisper-segmented content-free spans plus unit embeddings written into the sitting's work directory for the Rust authority to re-verify; the transcript text never leaves the worker process, the packaged internal-alpha operation set is unchanged and cross-pinned, and the op refuses on every runtime whose encoder is the placeholder). The store-side admission join exists too: `admit_derived_material` on the sitting authority verifies the whole worker join — capture-row raw digest, manifest encoder identity, embedding digest and one-per-scorable-segment count — before any durable write, consumes the work-directory artifacts durably ahead of the receipt-bound cleanup, and routes re-admission to cleanup retry. Next: the recording capture path itself (microphone into the work directory) and the derivation op's activation, both gated on encoder admission. The speaker-encoder packaging direction is decided (2026-08-03, `spike/encoder-packaging/RESULTS.md`): ONNX Runtime CPU execution is the preferred beta candidate with PyTorch/SpeechBrain as reference and fallback only; both admission checks are now largely measured — Fbank parity within 9.7 × 10⁻⁷ on synthetic and 7.4 × 10⁻⁷ on registered LibriSpeech fixtures, and a signed, notarized `build-alpha-encoder` bundle proven through the closed verifier, the hardened-runtime entitlement control, a bite-proven deny-network cold load, and peak memory beside MLX — but the gate-classification comparison (blocked on registered operating points from real calibration), the transferred-build Gatekeeper check, and the admission verdict itself remain open, so the default runtime's recorded encoder stays a placeholder and no real sitting may be claimed saved. Then record real sittings and permitted negative material, and let the operator select a measured operating point, without activating preserved bytes prematurely. Follow with withheld-turn restoration and stale-note regeneration. Exercise real destructive actions only as operator actions before beta admission. | Retention-policy wording and far-end-notice choices; the encoder admission verdict once the Fbank-parity and release-lane packaging checks record their results (the direction itself was chosen 2026-08-03: preferred ONNX candidate, voice isolation stays in the first beta); real migration, deletion, profile policy, reset, and withheld-turn decisions. | Cumulative 1–2 weeks |
| D. Evidence-linked automatic notes | In progress. The private coordinator and inspect-only transport remain closed and recovery-tested. The corrected synthetic-only MLX harness now advertises and parses the same strict `{"items":[...]}` contract, binds immutable model/runtime identities, separates tree hashing and model load from per-call timing, and fails closed to `transcript-only`. The bounded two-fixture Qwen2.5 1.5B corrective probe was rejected: the supported fixture produced invalid JSON and the empty fixture returned the wrong root/shape. **The one bounded repair this wave required was preregistered and run on 2026-08-05, and it passed.** Under structure-constrained decoding — a mask over the documented `logits_processors` hook, constraining shape only and leaving every value free so the locator gate stays able to fail — the same pinned model, prompt, and parser produced an accepted candidate on the supported fixture and the exact `{"items":[]}` abstention on the empty one, repeatable byte-for-byte across three cold runs. The accepted response is 181 generated tokens, the same count the rejected 2026-08-02 run recorded, at 344 bytes against its 345: both rejections were response shape, and neither measured comprehension. This admits nothing. **The fresh-process orchestrator was then built and the registered 12-fixture matrix ran on 2026-08-05: 36 processes, 60 calls, and it failed 9 of 12.** Its mechanical envelope is comfortable — a 5.79 s cold median against a 30 s ceiling, 4.39 s warm against 15 s, 1.18 GB peak against 4.28 GB — and every fixture is bit-for-bit repeatable across three cold processes. The failures are not the registered "schema-valid and wrong" outcome. Eight are one behaviour: the model emits `sf-` plus the fragment's own correct 64-hex digest and stops at 67 characters, the exact length and shape of the `candidate_id` format carried in the same request, so a well-formed identifier in the wrong format is refused as an unknown locator or, once padded to three copies, as a duplicate. **Why one fixture escapes this is unexplained** — enum absence, shape priming, and token alignment were each checked and each is refuted by the one success, whose request is structurally identical to a failure's. The ninth is a repetition loop the model produces byte-identically with the mask off, and is the only failure on this path the harness cannot be blamed for. Two harness defects are implicated and neither is fixed: `response_contract` never states three rules `_decode_response` enforces, and the fragment-ID field is the only identifier field not enumerated. SmolLM2's earlier result remains inconclusive. Neither model is admitted, no third candidate is scheduled, and no generator is wired into Preview. | Keep note work off the Preview critical path. **Corrected 2026-08-07: this cell stopped at the 2026-08-05 matrix and three registered interventions have run since, all on 2026-08-06.** It read "the next decision is which of two interventions to preregister — stating the unstated parser rules, or enumerating the offered fragment IDs". Both were run, and a third after them. The enumerated contract's matrix reached 8 of 12. Intervention three renamed one request key from `equals` to `rule`, leaving the surrounding prose byte-identical, and reached **9 of 12 passing every registered gate** with receipts (`notes/mlx_note_matrix_receipt_rule_key.json` and two siblings). The citation gate went from 7 of 10 to **10 of 10** — unpredicted, and the result worth keeping: the model had been reading the key, not the sentence. The three fixtures still refusing cite the exact canonical slice and fail on one mechanism, a 67-character identifier truncation, and the margin receipt shows one decision losing three times — same step, same runner-up token `-t`, at −1.11, −0.52 and −0.31 — rather than three coincidences. `admits` remains **false** and no generator is wired into Preview. Admit a create bridge only after typed output, exact locators, fidelity, latency, memory, repeatability, and human semantic/usefulness gates pass. | Semantic support and usefulness adjudication. | Additional 2–3 weeks after a registered experiment passes |
| E. Product surfaces and retrieval | In progress. The original light/editorial Preview now has a delivered Library → meeting detail → canonical transcript reader loop at `4598389`, with exact search, digest/path/parsed-byte rechecks, and a 960×900 visual acceptance size. Typed `note/2` claims and canonical claim evidence remain synthetic/contract evidence only; the projector and model path are not admitted to real Preview. The voice surface exposes content-free status, preserve-first legacy migration, separately confirmed lifecycle-backed reset, and the deterministic guided-enrolment shortfall; profile activation and the enrolment command remain unavailable, and the surface says which requirements it cannot decide. Correction/regeneration commands remain unregistered. **Two surfaces landed 2026-08-06.** § H first run now exists — six of its eight states over `capture/permission-probe`, a signed binary digest-verified through `app-runtime/1`, which is what made the screen buildable at all: nothing could previously measure either capture permission without recording, so any earlier first run would have had to claim states it never measured. It derives the step from a live measurement rather than a stored completion flag, keeps "we could not ask" a separate panel from "you said no", and omits `choose-retention` (wording awaits this wave's gate, and two of its drafted options have no encoding in `validate_start_request`) and `offer-calendar` (crosses the envelope) rather than stubbing them. Both request paths remain unexecuted anywhere, because running one outside the signed bundle answers about the wrong binary. And feature 6's restore control now appears on the screen shown right after a recording, not in Meetings only; that required `refresh_current_transcript`, since a restoration publishes a new current transcript and the recording screen has no reader to re-walk the way the Library route does. | Continue continual Preview delivery with the dedicated sitting recorder that feeds the guided-enrolment evaluator, then withheld-turn restoration and stale-note regeneration. Pass synthetic state tests and cold interaction review before beta admission. | Cold operator review of the working surfaces. | Additional 2–3 weeks, partly parallel with D |
...
| H. Later extensions | Outside v1: optional EventKit brief, detection, and conversational cross-meeting retrieval. **The operator-authored live note left this wave on 2026-08-06 by operator decision** and is recorded as an amendment in `product-definition.md`; §D is being built. Speaker playback/AEC remains research. | Each extension receives its own contract and evidence. | Separate scope and release decisions. | 1–4 weeks each after v1; no AEC ETA before feasibility passes |

### Transcript fidelity, readability, and evidence navigation

**Authority adopted now.** There is one canonical transcript: the closest
supported record of what was audibly said. Its faithful/verbatim-oriented policy
is explicit metadata: model and configuration digests, decoding, language,
timing/alignment mode, processing duration, and cold/warm state. A **Readable
transcript** is an optional derived presentation, never a competing record;
notes are interpretation. Every readable phrase and every note claim resolves
to canonical transcript text, and evidence links always land there.

**Next independent research, after the delivered reader slice.** Run an
isolated sidecar against public or synthetic audio only, using the existing
pinned MLX model with `word_timestamps=True`. It writes neither product nor
Preview records. Compare the current `condition_on_previous_text=False`
anti-poisoning baseline with one isolated continuation/seam arm; change nothing
unless registered long-form tests show fewer seam drops or duplicates without
error cascades. Quality is the gate: measure fillers, false starts, repeated
words, corrections, names, numbers, negation, silence hallucinations, looping,
early-EOT/truncation, seams, timing monotonicity, cold/warm latency, peak
memory, and repeatability. Smaller MLX variants may be packaging or latency
controls, never a quality substitute.

**Only if that sidecar passes measured thresholds and an operator finds
evidence-seek useful.** Add optional typed word timing—text, start, end,
physical channel, provenance, and decoded/aligned/interpolated label. Existing
character locators remain authoritative. First prove phrase audio seeking in a
disposable viewer with finite ordered times, the right channel and duration
bounds, exact locator resolution, and operator review. Then join deterministic
seam, silence, looping, fallback, and early-termination fixtures plus
content-free diagnostics (model identity/digest, duration, cold/warm, timing
mode, and failure category) into the permanent suite.

**Readable transcript follows, not precedes, fidelity proof.** Its reversible
view may visually suppress fillers or repeated fragments and normalize
punctuation, casing, and formatting. It must never silently change names,
dates, numbers, negation, commitments, or ambiguous corrections. One action
reveals the mapped canonical passage and, only if timing passed, seeks the
correct channel interval. Unmappable transformations are interpretation, like
generated notes. A cold operator comparison must measure reading speed, trust,
locator corrections, and usefulness before admission.

**Later research only.** Operator corrections may be reconciled to audio with
approximate alignment, but that label never strengthens evidence. An
operator-provided name/domain-term glossary or hotword path requires an
independently licensed local implementation and registered accuracy test.
There is no `verbatimize` requirement.

**Explicit non-adoptions.** No CrisperWhisper weights or outputs enter product
or Preview without a commercial license. There is no PyTorch runtime migration,
dual authoritative transcript, diarization, speaker identity, overlap
resolution, live transcription, or speaker-mode claim from this work. Approximate
alignment is not evidence, and no runtime, model, or signing change is justified
solely to demonstrate a concept testable in MLX.

**Joins and priority.** The MLX benchmark is next independent research. Join
only passing contract and test ideas into trust/evidence work; it does not block
Library, the note reader, trust controls, alpha receipts, or continual Preview
delivery. Schedule Readable Transcript only after the sidecar and evidence-seek
review pass. Any shipped feature still requires the existing human semantic,
...
descriptor-binds `profile` and inspects the lifecycle namespace before opening
or creating `voiceprint.json`. A published `lifecycle` requires the existing
live slot and refuses any simultaneous initializing directory; a missing live
slot is quarantined without repair. For a pristine profile with neither a live
slot nor either lifecycle directory, Rust create-new and full-syncs
`profile/.lifecycle.initializing` first, then create-new and full-syncs the safe
zero live slot. Only that retained initializing directory authorizes crash
resume and creation of a missing live slot. An existing `voiceprint.json` with
neither lifecycle directory is ambiguous between legacy material and total
journal loss, so automatic initialization refuses it; a separately reviewed
migration path must preserve or explicitly retire that legacy state. Rust then
builds both receipt files and the reset and enrolment zero slots under the
initializing directory, writes and full-syncs a sequence-zero `baseline`
receipt bound to the already pinned live descriptor, syncs the initializing
directory, then no-overwrite renames that exact retained directory to
`profile/lifecycle` with
...
the exact size and digest of future explicitly migrated material. A safe
semantically invalid migrated file may be recorded as present so reset can
remove it; the baseline does not admit it for Start. A crash before publication
leaves at most the fixed initializing directory and live sibling, and may
resume only if every present object matches the closed initialization shape.
Once `profile/lifecycle` exists, a valid baseline or later receipt is
mandatory. Missing fixed objects or two invalid/empty receipt slots quarantine
profile work. Removing the entire lifecycle leaves the live sibling but no
initializing authority and therefore refuses as ambiguous instead of silently
resetting receipt sequence or reset history.

The legacy decision is preserve-first. Three shapes were compared:

| Migration shape | Consequence | Decision |
|---|---|---|
| Automatically baseline any existing live file during startup | Fast, but erases the distinction between an expected legacy profile and missing lifecycle authority. | Rejected. |
| Delete the legacy file before lifecycle initialization | Removes ambiguity by destroying the only copy before the reset journal exists. | Rejected. |
| Operator chooses **Preserve for review**; Rust binds the exact safe live inode, size, and digest into the sequence-zero baseline without activating or changing it | Keeps the only copy, makes later validation or reset recoverable, and leaves capture under the existing one-operator limit. | Chosen. |

“Leave unchanged” is always available and performs no write. “Preserve for
review” is serialized by the process writer and global storage-sequence locks,
refuses while a meeting is active, and uses the same closed initializing shape
as pristine startup. A crash before the first receipt returns to migration
review; it cannot make a nonzero live slot self-authorizing. Once the receipt
binds the exact live identity, size, and digest, startup can finish publication
without asking again. Repeating the action after publication is idempotent. Preservation is not profile
validation or activation: strict semantic loading remains a later step. Profile
removal remains a separately confirmed reset action after lifecycle authority
exists; migration never hides deletion inside preservation.
...
current-effective-user-owned, exact `0700`, and has no extended ACL. The sole
permitted extended attribute is the observed macOS platform marker
`com.apple.provenance`, which carries no application authority; every other
attribute, including a resource fork, is refused. Each receipt or data slot is a current-effective-user-owned,
non-symlink regular file with one link, exact `0600`, `st_flags == 0`, no
extended ACL entries, and no extended attributes beyond that same platform marker. Profile
...
resource-fork attribute from hiding from the closed attribute test. Unsafe or
...
independent authority. The crate-private lifecycle transition, recovery matrix, and
strict-loader bridge are now implemented and tested; Preview exposure remains blocked
on the guided evidence-capture surface. Its `writing` and `ready` payloads have
`operation_id`, `requested_at`, `predecessor_reset` containing the completed
count and optional exact `last_reset` object, `live_zero`, and the exact untouched `reset_zero`;
...
serialized lifecycle transition. The Rust transition therefore remains crate-private
behind one coordinator: the canonical Python loader first admits one private candidate
and returns its digest; Rust descriptor-reopens that candidate, refuses extra entries,
re-derives the same digest, and publishes only those exact bytes. A caller assertion is
not profile authority. The digest-bound `profile.discard` worker operation is
idempotent. If cleanup is unavailable after `active`, the coordinator reports
`cleanup-pending` without rolling back or presenting enrolment as complete; a fresh
process can retry cleanup from the active receipt's operation ID and profile digest
without reopening or rewriting live profile bytes.
