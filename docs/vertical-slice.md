...
| E. Product surfaces and retrieval | In progress. The frozen Tauri correction/regeneration facade remains unregistered. An independently audited private Rust reader now rebuilds a bounded, nonpersisted projection from verified transcript-bearing meetings, performs exact normalized transcript search, returns content-free withheld hits, and reopens only projection-owned hits after fresh artifact reinspection. It makes no storage writes and has no Tauri command or UI. Title/folder metadata, claims, restored views, pagination, mutations, corpus bounds, and the product surfaces remain. | Add the hardened metadata reader and the remaining claim/retrieval behavior around this executable spine, then pass the synthetic corpus and cold interaction review before intentionally registering commands. | Cold operator review of the working surfaces. | Additional 2–3 weeks, partly parallel with D |
...
Current executable status: a private transcript-only Rust spine now implements
the first read path below. It verifies canonical meeting, attempt, static
lifecycle, deletion-reference, and current transcript identities; refuses a
partial projection at configured limits; performs the pinned exact Unicode
match; keeps withheld text out of hits and debug output; and re-inspects the
exact authority before opening a retained span. This bounded slice is not the
complete boundary: hardened `library/metadata.json`, claims and current-note
precedence, restored transcript views, paging and cursors, mutations, the
synthetic corpus benchmark, Tauri/UI registration, and cold review remain.
