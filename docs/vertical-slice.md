...
| E. Product surfaces and retrieval | In progress. The separate Preview bundle now registers the bounded reader commands and renders the real Library, exact search, meeting detail, typed `note/2` claims when present, canonical evidence landing, and transcript-only fallback in the original light/editorial design system. The frozen alpha command boundary is unchanged. The default 960×900 window shows the current Library without scrolling. Independent audit found two current interaction defects under correction: searches and return navigation can leave visible controls with superseded handles, and metadata-only search/fallback paths can offer a transcript action for meetings that have no transcript. Correction/regeneration commands remain unregistered. | Close the two audited handle/transcript-authority defects, rebuild the runnable Preview, then continue note-reader and trust-action slices. Pass the synthetic corpus and cold interaction review before beta admission. | Cold operator review of the working surfaces. | Additional 2–3 weeks, partly parallel with D |
