...
| E. Product surfaces and retrieval | In progress. The original light/editorial Preview now has a delivered Library → meeting detail → canonical transcript reader loop at `4598389`, with exact search, digest/path/parsed-byte rechecks, and a 960×900 visual acceptance size. Typed `note/2` claims and canonical claim evidence remain synthetic/contract evidence only; the projector and model path are not admitted to real Preview. The voice surface remains non-mutating until lifecycle-backed enrolment/reset exists; correction/regeneration commands remain unregistered. Storage coordination is the active next join. | Continue continual Preview delivery with storage coordination, lifecycle-backed profile enrolment/reset, then withheld-turn restoration and stale-note regeneration. Pass synthetic state tests and cold interaction review before beta admission. | Cold operator review of the working surfaces. | Additional 2–3 weeks, partly parallel with D |
