...
| C. Trust foundation | In progress. The independently audited restoration coordinator covers every durable phase and refuses simultaneous storage mutation. The development-only profile bridge validates quarantined candidates without following links, trusting unsafe ownership or modes, overwriting installed bytes, or crashing on malformed data. The macOS storage layer now has an independently audited, descriptor-bound profile primitive: full-chain no-follow directory opens, one-component `openat` files, closed owner/mode/link/flag/ACL/xattr/size checks, fixed zero-slot creation, identity observation, and namespace-replacement refusal. The core profile lifecycle builds and reopens the fixed sequence-zero baseline: it binds `$APP_DATA` through every slot, publishes only the retained initializing directory, requires canonical envelope and payload digests, distinguishes a torn peer from coherent later authority, detects same-length in-place mutation, and refuses missing published state without repair. Existing live material without lifecycle authority remains untouched until the operator explicitly chooses preserve-first migration. That action binds the exact safe bytes into the baseline without activation or deletion; an unreceipted crash returns to review, while a receipt-bound crash resumes safely. The raw initializer remains crate-private; its only dependent-crate capability borrows the process writer lock, holds the global storage sequence through completion, accepts no caller storage root, and refuses before mutation while any meeting is active. Preview invokes that authority once during startup and caches only a content-free result for Settings: baseline ready with profile presence, migration review required, needs attention, or unavailable. The installed baseline-ready Settings screen has been cold-reviewed. Opening Settings performs no lifecycle mutation or profile read. Migration and profile-attention states do not change startup, retention, capture admission, or current one-operator recording availability. Losing the bound app-data root or writer-lock authority remains a global startup failure rather than being mislabeled as profile damage. Reset and enrolment mutations remain unexposed. After explicit operator approval, the Preview also exposes a two-step, one-meeting audio-deletion action over the audited staged `audio-deletion/1` facade. A dedicated single-use handle, fresh retention recheck, process writer lock, and core meeting lease keep that action narrower than generic meeting or transcript authority. Synthetic tests prove released audio leaves transcript and note artifacts intact; the visible confirmation was opened and canceled against retained local data, so no real deletion receipt is claimed. The frozen alpha command boundary is unchanged. Alternating reset/enrolment phases, strict profile byte validation, guided enrolment/reset, withheld-turn restoration, policy change, and whole-meeting deletion remain. | Deliver the preserve-first migration action in Preview, then implement guided enrolment/reset without activating preserved bytes prematurely. Follow with withheld-turn restoration and stale-note regeneration. Exercise real destructive actions only as operator actions before beta admission. | Retention-policy wording and far-end-notice choices; real migration, deletion, profile policy, reset, and withheld-turn decisions. | Cumulative 1–2 weeks |
...
| E. Product surfaces and retrieval | In progress. The original light/editorial Preview now has a delivered Library → meeting detail → canonical transcript reader loop at `4598389`, with exact search, digest/path/parsed-byte rechecks, and a 960×900 visual acceptance size. Typed `note/2` claims and canonical claim evidence remain synthetic/contract evidence only; the projector and model path are not admitted to real Preview. The voice surface exposes only content-free status and preserve-first legacy migration; profile activation, reset, and enrolment remain unavailable. Correction/regeneration commands remain unregistered. | Continue continual Preview delivery with lifecycle-backed profile enrolment/reset, then withheld-turn restoration and stale-note regeneration. Pass synthetic state tests and cold interaction review before beta admission. | Cold operator review of the working surfaces. | Additional 2–3 weeks, partly parallel with D |
