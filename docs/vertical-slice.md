...
| D. Evidence-linked automatic notes | In progress. The private coordinator and inspect-only transport remain closed and recovery-tested. A synthetic-only MLX note-admission harness resolves exact canonical locators outside the model and fails closed to `transcript-only`. Two small pinned MLX candidates are now rejected. SmolLM2 1.7B repeated one malformed response; Qwen2.5 1.5B produced malformed non-JSON on all 60 registered synthetic calls and also missed the warm-latency ceiling. Citation, locator, semantic, and human gates were never reached. No generator or model is admitted or wired into Preview. | Audit the prompt/chat-template and constrained-output protocol before downloading another candidate. Admit a create bridge only after typed-output, exact-locator, latency, memory, repeatability, and human semantic/usefulness gates pass. | Semantic support and usefulness adjudication. | Additional 2–3 weeks after a registered experiment passes |
