...
| 4 | Evidence-linked notes | **Decide whether a contingent claim is refused or surfaced as contingent.** That is a product decision, not a harness one, and it is the operator's | Intervention eight ran and was **withdrawn the same day, 2026-08-07**, by its own preregistered falsifier (`notes/mlx_note_matrix_receipt_conditional_withdrawn.json`). A conditional gate refused `hypothetical-decision`, which carries its words, and left `conditional-unmarked` accepted, which does not — so it taught a vocabulary, not a distinction. **It also regressed two clean fixtures** (`ordinary-proposal`, `negation-proposal`), taking graded passes from 10 of 12 to 8 of 12, because advertising any rule moves every request on the path: all 13 pre-existing `request_sha256` values changed. **There is no targeted contract change here** — a gain on one fixture and a loss on two arrive as one edit, and none of it was attributable before intervention six added the request digest. Nothing now gates a conditional on either arm; a longer word list is spent, not promising. `admits` stays false. |
