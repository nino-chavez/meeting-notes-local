...
#
# `unsupported` takes `semantic-error` under DESIGN.md's rule that a warning needing
...
STATES = {
    "verified": ("check", "verified", "var(--semantic-success)",
                 "these words are in the transcript, at the turn shown"),
    "unsupported": ("cross", "not in the transcript", "var(--semantic-error)",
...
def check_locators(doc: dict, turns: list, note_path: Path) -> None:
    """Every verified claim's locator must land on the words it quotes.
    ...
    for claim in doc["claims"]:
        ...
        if i is None or not 0 <= i < len(turns):
            raise SystemExit(
                f"{note_path.name}: a verified claim points at turn {i}, which is not "
  ...
  <div class="split">
    <div class="col">
      ...
      {note_annotation("real data",
                       ...
                       "is unsupported, which is the one thing this surface exists to "
                       "show.")}
...
<section class="open">
  ...
  <ul>
    <li><strong>Settled: a claim's evidence state is part of the claim.</strong> Not a
      ...
      check, so a format that only renders verified evidence would have been designed
